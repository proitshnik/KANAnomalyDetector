...
if __name__ == "__main__":
    ...
    if plot_view_flag:
        ...
        predictor.plot_model_view(train_mode=True)
        ...
        if output_length < wanted_dot_num:
            ...
            for i in range(1, range_num + 1):
                ...
                if denormalize:
                    if not normalize_sequence:
                        real_output_denorm.extend(
                            np.round(predictor.denormalize_data(real_output).tolist()[output_length - step:], decimals=3)
                        )
                    else:
                        real_output_denorm.extend(
                            np.round(
    ...
    _log.info("Обучение завершено успешно!")
